...
export async function findClientByInstagramId(
  ...
  const { data, error } = await supabase
    ...
    // A connected Instagram account is NOT on its own permission to reply.
    // Three more gates, all of which were previously missing:
    //   lifecycle_status  — a draft client must never answer a real customer
    //                       before the founder has reviewed pricing, tone and
    //                       test conversations. Connecting Instagram during
    //                       setup used to be enough to go live by accident.
    //   is_paused         — Pause has to actually stop replies, immediately.
    //   billing_status    — a cancelled client stops replying at once, even
    //                       though their data is retained for the 60-day window.
